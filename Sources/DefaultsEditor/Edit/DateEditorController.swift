...
class DateEditorController: NPViewController {
	private let subject: CBSubject<Date>
	...
	private lazy var valueView: UIDatePicker = {
	...
	public init(subject: CBSubject<Date>) {
		...
		self.navigationItem.rightBarButtonItem = UIBarButtonItem(systemItem: .save, primaryAction: UIAction { [weak self] _ in
			...
			self.subject.send(self.valueView.date)
	...
	public override func viewDidLoad() {
		...
		self.valueView.date = self.subject.value
